# Bug Finder Configuration File Support

Configuration file support allows you to set default scan settings for the bug-finder tool, reducing command-line complexity and enabling reproducible scans with consistent parameters.
...
Solution: Validate JSON syntax. Use online JSON validator or `python -m json.tool config.json`
...
5. **Testing**: Test config files with `--dry-run` before full scans (when available)
...
- `bug-finder scan --help` - Full command help
- `bug-finder config-example --help` - Config generation help
